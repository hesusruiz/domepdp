...
func New(dbname string) (*TMFdb, error) {
	var err error
...
func (tmf *TMFdb) UpdateInStorage(dbconn *sqlite.Conn, po *pdp.TMFObject) error {
...
func (tmf *TMFdb) InsertInStorage(dbconn *sqlite.Conn, po *pdp.TMFObject) error {
...
func (tmf *TMFdb) UpsertTMFObject(dbconn *sqlite.Conn, po *pdp.TMFObject) (err error) {
	...
	// Get the resource name of object from the ID
...
func (tmf *TMFdb) RetrieveLocalTMFObject(dbconn *sqlite.Conn, href string, version string) (po *pdp.TMFObject, found bool, err error) {
	...
	po, err = pdp.NewTMFObject(oMap, content)
	if err != nil {
		log.Println(err)
		return nil, false, err
	}
...
func (tmf *TMFdb) RetrieveLocalListTMFObject(dbconn *sqlite.Conn, tmfType string, queryValues url.Values) (pos []*pdp.TMFObject, found bool, err error) {
	if dbconn == nil {
		dbconn, err = tmf.dbpool.Take(context.Background())
		if err != nil {
			return nil, false, err
